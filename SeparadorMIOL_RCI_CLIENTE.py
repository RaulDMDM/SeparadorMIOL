    ...
    for i in range(index, limit):
        ...
        if i+1 in termina_informe:
            try:
                ...
                print('-------------------------------------')
            with open("M:\\HOTFOLDERS\\SEPARADOR INSTANTANEO\\" + user +"\\%s - CLIENTE.pdf" % matriculas[matricula_index], "wb") as outputStream: # Ruta donde se van a guardar los pdfs con el nombre de la matricula (matriculas[matricula_index] es el array que contiene las matriculas)
                output.write(outputStream)
                outputStream.close()
                print('Creado correctamente: %s - CLIENTE' % matriculas[matricula_index])
