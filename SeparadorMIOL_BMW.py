...
for current_page in range(0, NumPages):
    ...
    if re.search(String, Text):
       ...
       sbstr_protocol = Text.split("\nComentarios:")
       protocol = sbstr_protocol[0].split("RaC:\n")
       protocolos.append(protocol[1])

       sbstr_vin = Text.split("  /  ")
       vin = sbstr_vin[1]
       bastidores.append(vin)
...
def crearArchivos(index, limit, matricula_index):
    ...
    for i in range(index, limit):
        ...
        if i+1 in termina_informe:
            if protocolos[matricula_index] == 'ALPHABET ESPAÑA FLEET MANAGEMENT SAU':
                try:
                    ...
                    print('-------------------------------------')
                with open("M:\\HOTFOLDERS\\SEPARADOR INSTANTANEO\\" + user +"\\%s.pdf" % bastidores[matricula_index], "wb") as outputStream: # Ruta donde se van a guardar los pdfs con el nombre de la matricula (matriculas[matricula_index] es el array que contiene las matriculas)
                
                    output.write(outputStream)
                    outputStream.close()
                    print('Creado correctamente: %s' % bastidores[matricula_index])
            else:
                ...
                try:
                    ...
                    print('-------------------------------------')
                with open("M:\\HOTFOLDERS\\SEPARADOR INSTANTANEO\\" + user +"\\R%s.pdf" % matriculas[matricula_index], "wb") as outputStream: # Ruta donde se van a guardar los pdfs con el nombre de la matricula (matriculas[matricula_index] es el array que contiene las matriculas)
